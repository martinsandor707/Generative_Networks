...
train_dataset = CustomDataset(root_dir="data", transform=T.Compose([
        T.ToPILImage(),
        T.Resize((64, 64)),
        T.ToTensor(),
...
class Generator(nn.Module):
    def __init__(self, input_dim, output_chs=3):
        super().__init__()
        ...
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(self.input_dim, 512, 4, stride=1, padding=0),
            nn.BatchNorm2d(512),
            nn.ReLU(True),
    ...
    def forward(self, x):
        ...
        return x[:, :, :64, :64] # Should not be needed but we double-check to make sure the outputs have the correct size
...
print(generator(torch.zeros((32, 100, 1, 1)).to(device)).shape)

print(discriminator(generator(torch.zeros((32, 100, 1, 1)).to(device))))

plt.imshow(generator(torch.zeros((128, 100, 1, 1)).to(device))[0].detach().cpu().permute(1, 2, 0)  * 0.5 + 0.5)
plt.show()
...
for epoch in range(num_epochs):
    ...
    for inputs, _ in tqdm(train_dataloader):
        ...
        output = discriminator(fake.detach()).view(-1) # You NEED detach() here
        ...
        output = discriminator(fake).view(-1) # You must NOT use detach() here
